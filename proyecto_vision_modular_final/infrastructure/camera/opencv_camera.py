...
class OpenCVCamera(ICamera):
    def __init__(self, index: int, width: int, height: int) -> None:
        ...
        self._width = width
        self._height = height
        self._cap = None  # se inicializa en open()

    def open(self) -> None:
        """
        Abre la cámara física y configura la resolución.

        Lanza RuntimeError si no se puede abrir.
        """
        try:
    ...
    def read(self) -> np.ndarray:
        """
        Lee un frame de la cámara ya abierta.

        Devuelve:
            frame (np.ndarray): imagen BGR.
        """
        if self._cap is None:
            print("[ERROR] Ha ocurrido un error con la cámara: no está inicializada.")
            raise RuntimeError("Cámara no inicializada. Llama primero a open().")
